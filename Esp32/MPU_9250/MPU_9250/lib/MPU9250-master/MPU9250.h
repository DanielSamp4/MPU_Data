...
private:
    ...
    void calibrate_acc_gyro_impl() {
        ...
        // initMPU9250();
        // delay(1000);
        //start the sequence with initializing MPU9250 and only afterwards set the values of the hardware registers... 
        initMPU9250();        // Toggle Reset Device --> clears ALL(!) registers --> also acc_gyro bias registers
        delay(1000);          // stabilize
        write_accel_offset(); // Push accelerometer biases to hardware registers
        write_gyro_offset();  // Push gyrometer biases to hardware registers
        delay(100);
    }
