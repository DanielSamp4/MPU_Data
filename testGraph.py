...
while True:
    ...
    # accel_x = np.random.random()
    # accel_y=np.random.random()
    # accel_z=np.random.random()
    # gyro_x=np.random.random()
    # gyro_y=np.random.random()
    # gyro_z =np.random.random()
    # data1 = np.random.random()  # Substitua por seus dados reais do MPU
    # data2 = np.random.random()  # Substitua por seus dados reais do MPU
    ...
    # Atualiza os dados y com o novo valor
    # y2[:-1] = y2[1:]  # Desloca todos os valores para a esquerda
    # y2[-1] = data2  # Insere o novo valor no final
    # line2.set_ydata(y2)
    ...
    fig2.canvas.draw()  # Redesenha a figura
    
    # Adiciona uma pequena pausa para controle de velocidade
    plt.pause(0.00001)

# Mantém o gráfico aberto após a conclusão
plt.ioff()
plt.show()
